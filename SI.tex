...
\begin{figure}
    \centering
    \includegraphics[width=0.6\linewidth]{images/GC_method_illustration.png}
    \caption{Illustration of Joback and Reid (JR) GC method}
    \label{fig:JR_GC_illustration}
\end{figure}

Figure \ref{fig:JR_GC_illustration} shows how the JR GC model in particular (and simple GC models in general) works. Each molecular group has a contribution to the total property value of the molecule. The group contributions $T_{b,i}$ for the normal boiling temperature $T_b$ are obtained as parameters from the JR GC model \cite{jobackEstimationPureComponentProperties1987}.
...
Figures \ref{fig:2D_outlier_analysis} and \ref{fig:outlier_scores} shows the results of outlier analysis on the entire datasets comparing predictions from the Joback and Reid (JR) GC method with experimental data. Either a power law or linear function (green line) was fitted to the JR GC predictions compared to experimental data, as shown in Figure \ref{fig:2D_outlier_analysis}. Two boundary lines (red lines) were then constructed representing two standard deviations of the JR GC predictions from the function fit line for each property.
...
Red points represent suspected outliers. These points were the focus of data quality checks. For all data points flagged by the We cross-checked the reported values in the CRC Handbook of Chemistry and Physics \cite{bookRumble2023} with values from Yaws' Critical Property Data for Chemical Engineers and Chemists \cite{yaws-critical-property} as available in the Knovel database. comparisons were also made with reported values in the National Institute of Standards and Technology (NIST) webbook as available. For all suspected outlier points, there was agreement between the CRC Handbook of Chemistry and Physics and  Yaws' Critical Property Data for Chemical Engineers and Chemists except for three molecules in the $\Delta H_{vap}$ dataset.
...
Figure \ref{fig:molecules_with_erroneous_data} shows the three molecules for which we consider the entries in the CRC Handbook of Chemistry and Physics to be incorrect.
We compared the values from the CRC Handbook of Chemistry and Physics with those from Yaws' Critical Property Data for Chemical Engineers and Chemists and data from NIST webbook. For the three molecules shown in Figure \ref{fig:molecules_with_erroneous_data}, the reported values from Yaws' Critical Property Data for Chemical Engineers and Chemists and those from NIST agree with each other while those from the CRC Handbook of Chemistry and Physics are significantly different.  


\begin{table}
    \centering
    \begin{tabular}{cccc}
         Cas RN&  T /K&  $\Delta H_{vap}$ /$kJmol^{-1}$& Reference
\\
         96-48-0&  357&  51.8& \cite{wibergLactones2Enthalpies1991}\\
         &  392&  49.5& \cite{wibergLactones2Enthalpies1991}\\
         &  407&  48.2& \cite{stephensonHandbookThermodynamicsOrganic1987}\\
         590-67-0&  355&  49.1& \cite{stephensonHandbookThermodynamicsOrganic1987}\\
         464-49-3&  298.15&  55.3& \cite{vanroonGasChromatographicDetermination2002}\\
    \end{tabular}
    \caption{$\Delta H_{vap}$ values compiled by National Institute of Standards and Technology (NIST) for molecules with erroneous $\Delta$$H_{vap}$ data from CRC Handbook of Chemistry and Physics. The collated data from NIST and Yaws' critical properties handbook data agree and differ from those of the CRC Handbook of Chemistry and Physics}
    \label{tab:NIST_Hvap_for_erroneous_CRC_Hvap}
\end{table}

Table \ref{tab:NIST_Hvap_for_erroneous_CRC_Hvap} shows data from NIST webbook as collated from the original references shown in Table \ref{tab:NIST_Hvap_for_erroneous_CRC_Hvap} for the three molecules shown in Figure \ref{fig:molecules_with_erroneous_data}.



\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{images/Hvap_outlier_analysis_scores.png}
    ...
    \label{fig:Hvap_outliers_removed}
\end{figure}

Figure \ref{fig:Hvap_outliers_removed} shows the outlier analysis results after the experimental data from the CRC Handbook of Chemistry and Physics for the three molecules in Figure \ref{fig:molecules_with_erroneous_data} have been replaced with those from Yaws' Critical Property Data for Chemical Engineers and Chemists. Interestingly, once the experimental data from the CRC Handbook of Chemistry and Physics for the three molecules in Figure \ref{fig:Hvap_outliers_removed} were replaced with the correct values from Yaws' Critical Property Data for Chemical Engineers and Chemists, the molecules were no longer flagged as outliers. 
...
There were still however three other molecules that remained flagged as outliers and whose data from the CRC Handbook of Chemistry and Physics were shown to be most likely correct as they agree with data from NIST and Yaws' Critical Property Data for Chemical Engineers and Chemists.
These three molecules are shown in Figure \ref{fig:JRGC_Hvap_bias_molecules}. It can be observed that these molecules are highly fluorinated or highly nitrated molecules. Detailed discussions have been provided in the main text about why the JR GC model significantly underestimate $\Delta H_{vap}$ for highly fluorinated molecules and overestimate $\Delta H_{vap}$ for highly nitrated molecules as shown in Figure \ref{fig:Hvap_outliers_removed}.

\subsubsection{Analysis of feature vs label data }
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{"images/MW_vs_Disc.png"} %Add 6 figure 2D plots of GP Data
...
\begin{figure}[H] %Move to SI
    \centering
    \includegraphics[width=\textwidth]{"images/MW_vs_Prop.png"} %Add 6 figure 2D plots of GP Data
...
\label{subsec:model_screening}
%Talk about 5 model forms
We implemented four separate GP model structures to improve the results of JR GC predictions. Three of the models (Models 1, 3, and 4) require both the GC property predictions $\mathbf{y_{GC}}$ and molecular weights ($\mathbf{M.W}$) and one (Model 2) only requires ($\mathbf{M.W}$) as an input. Each model makes different assumptions about the relationship between the input features and experimental property data $\mathbf{y_{\text{exp}}}$. We denote $\mathbf{X} = [\mathbf{y_{GC}}, \mathbf{M.W}]$

Model 1 described by equation \eqref{eq: Method_1} represents the most na\"ive approach to GP modeling in which we attempt to model $\mathbf{y_{\text{exp}}}$ using $m=0$. This method is best suited for models where trends in the data are unknown or difficult to elucidate and rely on the kernel function to fully describe trends in the data.


\begin{equation}
    \mathbf{y_{\text{exp}}} \sim  GP = \mathcal{N}\left(0, K(\mathbf{X}) \right)
    \label{eq: Method_1}
\end{equation}


Models 2 and 3 are represented by equations \ref{eq: Method_2} and \ref{eq: Method_3}. 
%attempt to model the discrepancy between the GC predictions and experimental data using a GP and are classified as traditional hybrid models. This method form is well-suited for properties where there is a clear correlation between the bias of the GC predictions and the experimental data. For a more robust implementation of this method, a mean function describing the bias with respect to input parameters should be added when clear. 
These models use $m = \mathbf{y_{GC}}$.
%which reflects our belief that for GC models which are fairly accurate, bias should be negligible. 
The difference between equations \ref{eq: Method_2} and \ref{eq: Method_3} is that equation \ref{eq: Method_2} models $\mathbf{y_{\text{exp}}}$ as a function of only $\mathbf{M.W}$ while equation \ref{eq: Method_3} models $\mathbf{y_{\text{exp}}}$ as a function of both $\mathbf{y_{GC}}$ and $\mathbf{M.W}$.

\begin{equation}
    \mathbf{y_{\text{exp}}} \sim GP = \mathcal{N}\left(\mathbf{y_{GC}}, K(\mathbf{M.W}) \right)
    \label{eq: Method_2}
\end{equation}
...
In this work, Model 3 is the final model implementation. 
%\begin{equation}
%    \mathbf{y_{\text{exp}}} - \mathbf{y_{GC}} = GP \sim \mathcal{N}\left(0, K(\mathbf{X}) \right)
%    \label{Method_3}
%\end{equation}

%The fourth GP model implementation (Model 3) in \eqref{Method_3} is the final model implementation which assumes $m = \mathbf{y_{GC}}$. 
This method is most suitable for property prediction data where $\mathbf{y_{GC}}$ is at least a somewhat faithful representation of $\mathbf{y_{\text{exp}}}$ and the relationship between $M.W$ and $\mathbf{y_{\text{exp}}}$ is unknown. 
%We note that because $\mathbf{y_{GC}}$ is not a stochastic function, Method 4 and Method 3 are mathematically equivalent. Therefore, Method 4 is a hybrid model and equivalent to method 3. These models also produce the same results within a small numerical tolerance. 

The fourth GP model form (Model 4) in equation \ref{eq: Method_4} is a generalization of \ref{eq: Method_3} which includes tunable hyperparameters $A$ and $b$ in the mean function such that $m(\mathbf{x} \vert \mathbf{A},\mathbf{b}) = \mathbf{A}\mathbf{x} + \mathbf{b}$. This model is well suited for property predictions where the correlation of $M.W$ and $\mathbf{y_{\text{exp}}}$ is linear or when $\mathbf{y_{GC}}$ is a poor estimator of $\mathbf{y_{\text{exp}}}$. 
...
The results of the examination of the effect of different kernels are provided in the main text. Here, we elaborate on the different kernels. For this section, $d(\mathbf{x}, \mathbf{x}^{\prime})$ is the euclidean distance between data $\mathbf{x}$ and $\mathbf{x}^{\prime}$.
...
\\
    \end{tabular}
    \caption{Final values of the optimized GP hyperparameters for each property}
...
\\
\\
         Variance hyperparameter setting&  $R^2$ &  MAPE&  MAE&  $R^2$ &  MAPE& MAE
\\
\\
         Start at 1$e^{-5}$ and optimize&  0.71&  13.14&  42.45&  0.71&  12.56& 42.09
\\
...
\\
    \end{tabular}
    \caption{Effect of different settings of the white kernel variance on the model training metrics for $T_m$. MAPE = Mean Absolute Percentage Error. MAE = Mean Absolute Error.}
    \label{tab:white_kern_settings_and_Tm}
\end{table}
...
\\
         perfluorodecalin&  $C_{10}F_{18}$&  306-94-5&  462.08&  9.68&  35.80&  35.8& 9.4
\\
         perfluorononane&  $C_{9}F_{20}$&  375-96-2&  488.07&  7.62&  34.22&  35.8& 13.9
\\
         perfluoro-2-methylpentane&  $C_{6}F_{14}$&  355-04-4&  338.05&  9.74&  27.89&  25.7& 16.3
\\
         pentadecafluoro-octanoic acid&  $C_{8}HF_{15}O_{2}$&  335-67-1&  414.07&  29.24&  40.40&  41.3& 19.9
\\
         heptadecafluoro-nonanoic acid&  $C_{9}HF_{17}O_{2}$&  375-95-1&  464.08&  28.54&  43.03&  45.1& 22.7
\\
\end{tabular}
...
\begin{table}[H]
    \centering
    \begin{tabular}{>{\raggedright\arraybackslash}p{2.2cm}>{\raggedright\arraybackslash}p{1.20cm}>{\centering\arraybackslash}p{1.20cm}>{\centering\arraybackslash}p{1.10cm}>{\centering\arraybackslash}p{1.10cm}>{\centering\arraybackslash}p{1.10cm}>{\centering\arraybackslash}p{1.10cm}>{\centering\arraybackslash}p{1.10cm}>{\centering\arraybackslash}p{1.10cm}>{\centering\arraybackslash}p{1.10cm}>{\centering\arraybackslash}p{1.10cm}}
